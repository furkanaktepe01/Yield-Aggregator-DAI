...
class App extends Component {
	...
	async loadAccountInfo() {    console.log(this.state.aggregator.methods.amountDeposited().call()); console.log("called")

		const walletBalance = this.state.web3.utils.fromWei(await this.state.dai.methods.balanceOf(this.state.account).call(), 'ether') 
		const aggregatorBalance = this.state.web3.utils.fromWei(await this.state.aggregator.methods.amountDeposited().call(), 'ether') ;  
